...
simPrcpFile  = "PRCP_%04d%02d%02d%02d.gt"

dscgDir      = "/data4/yuta/camaOut/MSM5_newriv/%04d/%02d/%02d/%02d/"
...
def hyetohydro():
    ...
    simPrcp = basinPrcp(simPrcpDir,simPrcpFile,simExecStep,FT,startDate,endDate)

    cumObs  = obsPrcp.sum(axis=0)
    cumSim  = simPrcp.sum(axis=0)

    PCC      = corrCoef(cumObs.flatten(),cumSim.flatten())
...
#    map.imshow(np.ma.masked_equal(cumSim[ys:ye,xs:xe],0),origin="upper",cmap=cm.CMRmap_r,alpha=0.7,zorder=2)
    cbar=plt.colorbar(shrink=0.7,extend="max")
    cbar.ax.tick_params(labelsize=20)
    plt.title("MSM [39h cumrative] IT:"+startDate.strftime("%Y/%m/%d %H:00")+"[UTC]")
    plt.savefig("./figure/MSM_cum_"+startDate.strftime("%Y%m%d%H"))
    plt.show()
    ...
    cbar.ax.tick_params(labelsize=20)
    plt.title("MSM - RDR [39h cumurative] IT:"+startDate.strftime("%Y/%m/%d %H:00")+"[UTC]")
    plt.savefig("./figure/MSM-RDR_cum_"+startDate.strftime("%Y%m%d%H"))
    plt.show()
